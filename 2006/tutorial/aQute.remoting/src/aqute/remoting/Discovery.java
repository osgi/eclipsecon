...
public class Discovery extends Thread {
	...
	Discovery(Remoting remoting) throws UnknownHostException {
	...
	private void announce() throws Exception {
		ServiceReference references[] = remoting.remotedServices
		...
		parms[1] = new Integer(remoting.server.getLocalPort());
		parms[2] = properties;
	...
	void discover(InetAddress address, Message message) throws IOException {
		...
		if (host.equals(remoting.server.getInetAddress())
				&& port == remoting.server.getLocalPort())
			return;

		Properties[] properties = (Properties[]) parms[2];
		RemoteHost rhost = remoting.getHost(host, port);
